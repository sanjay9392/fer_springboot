...
<tr><td> <div class="timer-run">00:00</div> </td></tr>
</table>
	<jsp:include page="participantsList.jsp"></jsp:include>
	</td></tr>
</table>
<div id="footer">
...
var utype = "A";//"<?php echo $this->session->userdata('utype'); ?>";
var isAOBanker = (utype=="A") ? true : false;
var isClosingTimer = "";
var curCloseTimer = 0;
//alert(isAOBanker);
var timerSync = 2000;
...
function showNetworkError() {
	$(".network-status").html("Network Slow or Down").addClass("network-error");
}
...
function updateCurrentBid(data) {
	...
	if(data.found || data.found=="true") {
		ajaxRespNo = data.ajaxRespNo;
		if(ajaxRespNo == ajaxReqNo) $(".network-status").html("Network Connected11").removeClass("network-error");
...
function updateAuctionStatus(data) {
	...
	if(data.status=="S") { statusStr = "Running"; isAuctionStarted = true; $(".button5").hide(); }
	if(data.status=="C") {
		statusStr = "Closed";
		showMessage("Auction Info", "Highest Bid is Rs."+displayMoney(data.currentBid.bidAmt), "<input type='button' class='button3' value='OK' onclick='msgOK()'> &nbsp; <input type='button' class='button' value='Print Report' onclick='openWindow("+data.aid+")'>");
		clearInterval(bidInterval);
...
function closeAuctionByAO() {
	...
	$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>admin/BankProjects/ajaxCloseAuction/"+aid,
...
function getDateObjectByOffset(dtStr) {
	...
	//var dt2 = new Date(t[0], t[1]-1, t[2], parseInt(t[3],10) + 5, parseInt(t[4], 10) + 30, t[5], 0);
	return dt2;
}
