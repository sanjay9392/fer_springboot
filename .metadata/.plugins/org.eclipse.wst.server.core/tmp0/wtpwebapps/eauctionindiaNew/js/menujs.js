...
var csse3 = function () {
    ...
    return {
        initMenu: function () {
        	var m = document.getElementById("csses3");
        	if (!m) return;
            m.style.width = m.getElementsByTagName("ul")[0].offsetWidth + 1 + "px";
            var lis = m.getElementsByTagName('li');
            var a = m.getElementsByTagName("a");
...
if (window.addEventListener) {
    ...
    window.addEventListener("load", csse3.initMenu, false);
}
else if (window.attachEvent) {
	window.attachEvent("onload",sse3.initMenu);
   window.attachEvent("onload", csse3.initMenu);
}
else {
    window["onload"] = sse3.initMenu;
    window["onload"] = csse3.initMenu;
}
